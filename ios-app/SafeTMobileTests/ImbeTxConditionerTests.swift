...
///
/// Two recent fixes (Swift exclusivity violation in `conditionLe16`,
/// stored-property initializer for CI) plus the "clear IMBE uplink buffer
/// after busy/abort" path make this a regression-prone spot. The tests
/// below pin both the short/malformed-input guards and the stateful
/// `reset()` contract that VoiceTransport.resetUplinkState() relies on.
final class ImbeTxConditionerTests: XCTestCase {
    ...
    /// it stays in place and the byte is left untouched (no read past end).
    func test_conditionLe16_singleByteFrame_isLeftUnchanged() {
        let conditioner = ImbeTxConditioner()
        var frame = Data([0x42])
        conditioner.conditionLe16(frame: &frame)
        XCTAssertEqual(Array(frame), [0x42])
    }
    ...
    /// Slamming the input at full positive scale exercises the soft-knee
    /// limiter: the output must never exceed ±32760 and must never overflow
    /// Int16 (`Int16(clamping:)` would otherwise raise on the bad path).
    ...
    /// Mirror of the positive-extremes case for the negative full-scale
    /// edge — separate so a single-side clamp regression is obvious.
    ...
    func test_conditionLe16_loudToneRemainsBounded_acrossManyFrames() {
        let conditioner = ImbeTxConditioner()
        let toneHz = 1_000.0
        let fs = 16_000.0
        var phase = 0.0
    ...
    func test_reset_returnsToSilentPassthrough_afterLoudActivity() {
        let conditioner = ImbeTxConditioner()
        ...
        for sample in readLe16Samples(silent) {
            XCTAssertEqual(sample, 0, "post-reset silence must remain silent")
        }
    }
    ...
    private func assertAllSamplesInRange(_ frame: Data,
