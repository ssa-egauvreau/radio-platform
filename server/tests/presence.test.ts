 ...
 *     labels with slightly different whitespace), and tolerates the
 *     `unknown` input it accepts straight off the request body.
 *   - `heartbeatPresence` rejects empty / sentinel-`----` channel
 *     values and empty unit IDs before they pollute the map (the
 *     legacy "no channel" placeholder must never get a count).
 *   - Unit IDs are upper-cased so `u-1` and `U-1` never double-count
 *     the same physical handset.
 ...
 *     double-counting.
 *   - Partial expiry: one unit on a channel times out, another that
 *     re-heartbeated mid-window survives.
 ...
 *
 * Time is advanced via `node:test` mock timers (or a tiny stub of
 * `Date.now`) so the TTL boundary is tested deterministically.
 */
...
/**
 * Drop every heartbeat the previous test left behind. Presence intentionally
 * has no exported "clear" — but the internal prune in `countPresence` removes
 * every entry whose timestamp is older than TTL_MS. We pin `Date.now` an
 * hour into the future to guarantee the prune fires over every existing
 * entry from prior tests, then restore the real clock. Calling a few
 * disjoint agencies ensures the prune walks the whole outer map.
 */
...
// Each test that doesn't need a specific agency id picks a unique one out of
// this pool so it stays independent of other tests that leak state into the
// process-global map (the tests below all call clearPresence(), but this is
// belt-and-braces for any future test that forgets).
