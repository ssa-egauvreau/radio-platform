...
export function createApiRouter(): Router {
  ...
  router.get("/audio/config", requireAgencyMember, async (req, res) => {
    try {
      ...
      // sibling `audioConfig.ts` / `audioConfigDevice.ts` modules are kept
      // for their own regression test suites; the route uses the canonical
      // one with the typed `GlobalAudioLabConfigPreImbe` outer shape.
