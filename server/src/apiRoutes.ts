...
// `deriveDeviceAudioConfig` ships in three parallel-extracted modules
// (`audioConfig.ts`, `audioConfigDevice.ts`, `audioConfigDerive.ts`) — each
// has its own test file pinning the same mapping rules. The route uses
// `audioConfig.ts` because it accepts the raw stored `row.config` JSON
// directly (no caller-side `.preImbe` destructure / cast).
