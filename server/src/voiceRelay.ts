...
/**
 * Subset of a {@link MoveLockRosterRecord} that
 * {@link computeUnitChannelCounts} cares about. Broken out so the counting
 * rule can be exercised in unit tests without spinning up a WebSocket server
 * to seed the live roster.
 */
 ...
 *
 * Only console-style `account` sessions count (`device_type` explicitly marked
 * `dispatch_console`, or web/desktop sessions where `device_type` is still
 * missing during join hydration). A user who just has a handset/phone on one
 * channel and the dashboard open on another must still be drag-droppable.
 * Multi-channel scanning is a dispatch-console signal, not a
 * "this person is everywhere" signal.
 */
