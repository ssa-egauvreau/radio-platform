...
// dvmvocoder so the recorder can store digital transmissions as plain PCM.
...
/**
 * Decodes a 13-byte IMBE frame (2-byte marker + 11-byte codeword) to 16 kHz
 * mono PCM-16, or null if the vocoder is unavailable.
 */
export function decodeImbeFrame(frame: Buffer): Buffer | null {
  const mod = codec;
  if (!mod || frame.length !== 13) {
    return null;
  }
