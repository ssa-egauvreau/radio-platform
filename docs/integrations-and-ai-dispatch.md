...
- **TTS model** — Server default `eleven_turbo_v2_5` (real-time, API-accessible) with **Creative** stability (`0.0`). Optional Railway overrides: `ELEVENLABS_MODEL_ID`, `ELEVENLABS_STABILITY` (`0` Creative, `0.5` Natural, `1` Robust). Note: `eleven_v3` is alpha and not reachable through the public TTS API — setting it silences the dispatcher unless your ElevenLabs account has v3 API access.
